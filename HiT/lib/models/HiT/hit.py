...
class HiT(nn.Module):
    ...
    def __init__(self, backbone, box_head, hidden_dim, num_queries,
                 bottleneck=None, aux_loss=False, head_type="CORNER", neck_type='LINEAR', template_preprocess=None):
    ...
    def forward(self, images_list=None, xz=None, mode="backbone", run_box_head=True, run_cls_head=False, template_anno=None):
        if mode == "backbone":
            if self.template_preprocess is not None:
                ...
                if type(self.template_preprocess) is Draw:
                    images_list[1] = self.template_preprocess(images_list[1], template_anno)
...
def build_hit(cfg):
    ...
    pretrained_ckpt = getattr(cfg.MODEL, "PRETRAINED", "")
    print("pretrain<<<<<<", pretrained_ckpt)
    if pretrained_ckpt is not "":
