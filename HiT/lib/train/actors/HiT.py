...
class HiTActor(BaseActor):
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        feature_xz = self.net(images_list=images_list, mode='backbone', template_anno=data['template_anno'][0],)

        out_dict, _, _ = self.net(xz=feature_xz, mode="head", run_box_head=run_box_head, run_cls_head=run_cls_head, template_anno=data['template_anno'][0],)
        return out_dict
