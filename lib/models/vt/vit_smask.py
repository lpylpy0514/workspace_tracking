#vit_mask with 2d positional embedding and 1d time emedding
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        self.time_embed = nn.Parameter(torch.zeros(1, self.num_template+self.num_search, embed_dim))
        time_embed = get_sinusoid_encoding_table(self.num_template+self.num_search, self.time_embed.shape[-1], cls_token=False)
    ...
    def forward_features(self, images_list):
        ...
        for i in range(num_search):
            ...
            x = x + self.pos_embed[:, 0:self.num_patches_search, :]
            x_list.append(x)
        x_feat = torch.cat(x_list, dim=1)
        xz_feat = torch.cat([x_feat, z_feat], dim=1)

        B = xz_feat.shape[0]
        time_embed_t = self.time_embed[:,0:1,:]
        time_embed_t = time_embed_t.unsqueeze(2).expand(B, -1, self.num_patches_template, -1).flatten(1,2)
        time_embed_s = self.time_embed[:,1:num_search+1,:]
        time_embed_s = time_embed_s.unsqueeze(2).expand(B, -1, self.num_patches_search, -1).flatten(1,2)
        time_embed = torch.cat([time_embed_s, time_embed_t],dim=1)
        xz_feat += time_embed
        xz = self.pos_drop(xz_feat)

        attn_mask = generate_encoder_mask(num_search, num_template,
                                          self.num_patches_search, self.num_patches_template).to(xz.device)
...
def generate_encoder_mask(num_search, num_template, ele_search, ele_template):
    mask = (torch.triu(torch.ones(num_search, num_search)) == 1).transpose(0, 1)
    mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
    mask = mask.reshape([num_search,1,num_search,1])
    mask = mask.expand(-1,ele_search,-1,ele_search)
    mask = mask.flatten(0,1).flatten(1)
    mask_template_padd_bottom = torch.zeros([num_template*ele_template, num_search*ele_search])
    mask_template_padd_bottom = mask_template_padd_bottom.masked_fill(mask_template_padd_bottom == 0, float('-inf'))
    mask = torch.cat([mask,mask_template_padd_bottom], dim=0)
    mask_template_padd_right = torch.zeros([num_template*ele_template+num_search*ele_search, num_template*ele_template])
    mask = torch.cat([mask,mask_template_padd_right], dim=1)
    return mask
...
@register_model
def vit_smask_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vit_smask_base_patch16_224_' + pretrain_type
...
@register_model
def vit_smask_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_smask_large_patch16_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
@register_model
def vit_smask_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_smask_huge_patch14_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
def load_pretrained(model, pretrain_type='default', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    ...
    state_dict['time_embed'] = model.state_dict()['time_embed']
