...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
                 ...
                 search_number=1, template_number=1, global_layers=None, local_size=16):
        super().__init__()
    ...
    def forward_features(self, images_list):
        ...
        for i in range(number_images):
            ...
            if i == 0:
                ...
                xz_list.append(x)
        xz = torch.cat(xz_list, dim=1)
        xz = self.pos_drop(xz)
        B, _, D = xz.shape
        # global --> local
        len_feat_search = int(self.num_patches_search ** 0.5)
        len_window_search = len_feat_search // self.local_size
        num_window_search = len_window_search ** 2
        len_feat_template = int(self.num_patches_template ** 0.5)
        len_window_template = len_feat_template // self.local_size
        num_window_template = len_window_template ** 2
        feat_local_list = []
        for i in range(number_images):
            if i == 0:
                x = xz[:,0:self.num_patches_search,:]
                x = x.reshape([B, len_feat_search, len_feat_search,D])
                x = x.reshape([B, len_window_search, self.local_size, len_window_search, self.local_size, D]).permute(0,1,3,2,4,5).flatten(1,2)
                feat_local_list.append(x)
            else:
                x = xz[:,(self.num_patches_search+(i-1)*self.num_patches_template):(self.num_patches_search+i*self.num_patches_template),:]
                x = x.reshape([B, len_feat_template, len_feat_template,D])
                x = x.reshape([B, len_window_template, self.local_size, len_window_template, self.local_size, D]).permute(0,1,3,2,4,5).flatten(1,2)
                feat_local_list.append(x)
...
@register_model
def vit_local_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vit_local_base_patch16_224_' + pretrain_type
...
@register_model
def vit_local_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_local_large_patch16_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
@register_model
def vit_local_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_local_huge_patch14_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
