...
class VTO(VT):
    ...
    def __init__(self, backbone, box_head, hidden_dim, num_queries,
                 aux_loss=False, head_type="CORNER", cls_head=None, iou_head=None):
        """ Initializes the model.
        VTO including the VTM's cls_head and the VTT's iou_head
        VTO also integrates search region and template into one image before backbone network
        """
    ...
    def forward(self, images_list=None, xz=None, mode="backbone",
                run_box_head=False, run_cls_head=False, run_iou_head=False):
        if mode == "backbone":
            return self.forward_backbone(images_list)
        elif mode == "transformer": # this is head not transformer
            return self.forward_transformer(xz,
                                            run_box_head=run_box_head,
                                            run_cls_head=run_cls_head,
                                            run_iou_head=run_iou_head)
        else:
            raise ValueError
    ...
    def forward_transformer(self, xz, run_box_head=False, run_cls_head=False, run_iou_head=False):
        #chenxin
        # self.adjust(xz)
        if self.aux_loss:
        ...
        out, outputs_coord = self.forward_head(output_embed, x_mem,
                                               run_box_head=run_box_head,
                                               run_cls_head=run_cls_head,
                                               run_iou_head=run_iou_head)
        return out, outputs_coord, output_embed

    def forward_head(self, hs, memory, run_box_head=False, run_cls_head=False, run_iou_head=False):
        """
