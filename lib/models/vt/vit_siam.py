...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        self.pos_embed_search = nn.Parameter(torch.zeros(1, self.num_patches_search + 1, embed_dim))
        self.pos_embed_template = nn.Parameter(torch.zeros(1, self.num_patches_template + 1, embed_dim))
    ...
    def no_weight_decay(self):
        return {'pos_embed_search', 'pos_embed_template', 'cls_token'}
    ...
    def forward_features(self, image_list):
        mode = image_list[0]
        image = image_list[1]
        if mode == "search":
            pos_embed = self.pos_embed_search
        elif mode == "template":
            pos_embed = self.pos_embed_template
        else:
            raise ValueError("illegal mode")
        B = image.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = self.patch_embed(image)
        cx = torch.cat((cls_tokens, x), dim=1)
        cx = cx + pos_embed
        cx = self.pos_drop(cx)
        ...
        cx = self.norm(cx) # torch.Size([16, 721, 768]) B,N,C
        return cx

    def forward(self, image_list):
        cx = self.forward_features(image_list)
        # x = self.head(x)
        return cx
...
@register_model
def vit_siam_base_patch16(pretrained=False, pretrain_type='default',
...
@register_model
def vit_siam_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
...
@register_model
def vit_siam_huge_patch14(pretrained=False, pretrain_type='default',
...
def load_pretrained(model, pretrain_type='default', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    ...
    if side_pe != side_num_patches_template:
        ...
        pe_t = pe
    pe_cs = torch.cat((cls_pe, pe_s), dim=1)
    pe_ct = torch.cat((cls_pe, pe_t), dim=1)
    state_dict['pos_embed_search'] = pe_cs
    state_dict['pos_embed_template'] = pe_ct
