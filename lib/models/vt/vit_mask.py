...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        self.num_patches_per_unit =  self.num_patches_search + self.num_patches_template * self.num_template
        ...
        self.time_embed = nn.Parameter(torch.zeros(1, self.num_search, embed_dim))
        time_embed = get_sinusoid_encoding_table(self.num_search, self.time_embed.shape[-1], cls_token=False)
    ...
    def forward_features(self, images_list):
        ...
        for i in range(num_search):
            ...
            xz_list.append(xz)
        xz_feat = torch.cat(xz_list, dim=1)
        B = xz_feat.shape[0]
        time_embed = self.time_embed[:,0:num_search,:]
        time_embed = time_embed.unsqueeze(2).expand(B, -1, self.num_patches_per_unit, -1).flatten(1,2)
        xz_feat += time_embed
        xz = self.pos_drop(xz_feat)

        attn_mask = generate_encoder_mask(xz.shape[1], num_search).to(xz.device)
...
def generate_encoder_mask(sz, num_frames):
    num_feat = sz // num_frames
    mask = (torch.triu(torch.ones(num_frames, num_frames)) == 1).transpose(0, 1)
    mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
    mask = mask.reshape([num_frames,1,num_frames,1])
    mask = mask.expand(-1,num_feat,-1,num_feat)
...
@register_model
def vit_mask_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vit_mask_base_patch16_224_' + pretrain_type
...
@register_model
def vit_mask_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_mask_large_patch16_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
@register_model
def vit_mask_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_mask_huge_patch14_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
