...
class TargetMask(nn.Module):
    """ The target mask to high-light the target in template.
    """
    def __init__(self, embed_dim=768):
        super().__init__()
        self.fore_embedding = nn.Parameter(torch.ones(1, embed_dim))
        self.back_embedding = nn.Parameter((1e-4)*torch.ones(1, embed_dim))
    def get_target_mask(self, x, anno):
        b, hw, d = x.shape
        h = w = int(hw**0.5)
        #generate the segment mask from bbox
        x1 = torch.max(anno[:,0] * w, torch.tensor([0.]).to(anno)).floor().int()
        y1 = torch.max(anno[:,1] * h, torch.tensor([0.]).to(anno)).floor().int()
        x2 = torch.min((anno[:,0] + anno[:,2]) * w, torch.tensor([w]).to(anno)).ceil().int()+1
        y2 = torch.min((anno[:,1] + anno[:,3]) * h, torch.tensor([w]).to(anno)).ceil().int()+1
        blank_mask = torch.zeros(b, h, w).to(x.device)
        for i in range(b):
            blank_mask[i,y1[i]:y2[i],x1[i]:x2[i]] = 1
        #fill the mask with embedding
        blank = torch.zeros(b, h, w, d).to(x.device)
        index = torch.nonzero(blank_mask == 1)
        blank = torch.index_put(blank, (index[:, 0], index[:, 1], index[:, 2]), self.fore_embedding)
        index = torch.nonzero(blank_mask == 0)
        blank = torch.index_put(blank, (index[:, 0], index[:, 1], index[:, 2]), self.back_embedding)
        blank = blank.view(b,-1,d)
        return blank

    def forward(self, x, anno):
        target_mask = self.get_target_mask(x, anno)
        x = x * target_mask
        # x = x.view(b,h,w,-1)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        self.target_mask = TargetMask(embed_dim=embed_dim)
    ...
    def forward_features(self, item_list):
        template_anno = item_list[-1]
        images_list = item_list[0:-1]
        ...
        for i in range(len(images_list)):
            ...
            if i == 0:
                ...
                x = self.target_mask(x, template_anno[i-1])
...
@register_model
def vitfull_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vitfull_base_patch16_224_' + pretrain_type
...
@register_model
def vitfull_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vitfull_large_patch16_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
@register_model
def vitfull_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vitfull_huge_patch14_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
def load_pretrained(model, pretrain_type='default', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    ...
    state_dict['target_mask.fore_embedding'] = model.state_dict()['target_mask.fore_embedding']
    state_dict['target_mask.back_embedding'] = model.state_dict()['target_mask.back_embedding']
