...
default_cfgs = {
    ...
    'vit_mtr_base_patch16_256_v_b_16_256_bs64_20m_e5_nl_imn1k_m60_2d': _cfg(
        url='https://azsusw3.blob.core.windows.net/v-zhaojie/projects/playground/playground/checkpoints/train/mtr/v_b_16_256_bs64_20m_e5_nl_imn1k_m60_2d/MTR_ep0005.pth.tar?sv=2020-10-02&st=2022-04-28T01%3A03%3A38Z&se=2025-04-29T01%3A03%3A00Z&sr=c&sp=racwl&sig=juG7219CQ5wQFVIE5dWpNcaXilZQ9h7bBW42KwKI6oQ%3D'
    ),
}
...
@register_model
def vit2d_mtr_xs_patch16(num_img=2,
...
@register_model
def vit2d_mtr_base_patch16(num_img=2,
                         pretrained=False, pretrain_type='scratch',
                         search_size=256, template_size=256, **kwargs):
    patch_size = 16
    model = VisionTransformer(
...
@register_model
def vit2d_mtr_large_patch16(num_img=2,
                          pretrained=False, pretrain_type='mtr',
                              search_size=256, template_size=256, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        num_img=num_img,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_mtr_large_patch16_256'
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
@register_model
def vit2d_mtr_huge_patch14(num_img=2,
                         pretrained=False, pretrain_type='mtr',
                        search_size=256, template_size=256, **kwargs):
    patch_size = 14
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_mtr_huge_patch16_256'
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
def load_pretrained(model, pretrain_type='default', cfg=None, filter_fn=None, strict=True):
    ...
    model_dir = os.path.join(os.path.dirname(__file__),'../../..','pretrained/mtr', pretrain_type)
