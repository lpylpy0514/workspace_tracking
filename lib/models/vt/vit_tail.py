...
class CrossAttention(nn.Module):
    #implemented by chenxin to compute cross attention; x-->q z-->kv
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
...
class Tail(nn.Module):
    # implemented by chenxin to integrate search region and template to one tensor. the shape is same with search region
    def __init__(self, num_x, num_z, dim, num_heads,
    ...
    def forward(self, xz):
        xz = self.norm1(xz)
        xc = xz[:,0:self.num_x+1,:] #search region and cls token
        z = xz[:,self.num_x+1:,:] #multiple template
        xc = xc + self.drop_path(self.attn(xc, z))
        xc = xc + self.drop_path(self.mlp(self.norm2(xc)))
        return xc
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        self.tail = Tail(num_x=self.num_patches_search,
                         num_z=self.num_patches_template,
    ...
    def forward_features(self, images_list):
        ...
        xz = self.tail(xz)
        for blk in self.blocks[1:]:   #vit use the their implemented attention,so the batchszie is the first.
            xz = blk(xz)
...
@register_model
def vittracking_tail_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vittracking_tail_base_patch16_224_' + pretrain_type
...
@register_model
def vittracking_tail_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vittracking_tail_large_patch16_224_' + pretrain_type
    model.default_cfg = default_cfgs[cfg_type]
    if pretrained:
...
@register_model
def vittracking_tail_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vittracking_tail_huge_patch14_224_' + pretrain_type
    model.default_cfg = default_cfgs[cfg_type]
    if pretrained:
...
def load_pretrained(model, pretrain_type='default', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    ...
    for param_name in model.state_dict().keys():
        if 'tail' in param_name:
            if '.q.' not in param_name and '.kv.' not in param_name:
                param_name_load = param_name.replace('tail','blocks.0')
                state_dict[param_name] = state_dict[param_name_load]
    attn_dim = model.state_dict()['tail.attn.q.weight'].shape[0]
    state_dict['tail.attn.q.weight']  = state_dict['blocks.0.attn.qkv.weight'][0:attn_dim, :].clone()
    state_dict['tail.attn.kv.weight'] = state_dict['blocks.0.attn.qkv.weight'][attn_dim:, :].clone()
    state_dict['tail.attn.q.bias'] = state_dict['blocks.0.attn.qkv.bias'][0:attn_dim].clone()
    state_dict['tail.attn.kv.bias'] = state_dict['blocks.0.attn.qkv.bias'][attn_dim:].clone()
    # strict = False #close the strict by chenxin to load params except tail
