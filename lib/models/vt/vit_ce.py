#vit_mask with 2d positional embedding, mask = none, a clean vit
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192,
                 ...
                 search_number=1, template_number=1, use_checkpoint=False, ce_loc=None, ce_keep_ratio=None):
        super().__init__()
    ...
    def forward_features(self, images_list):
        ...
        for i, blk in enumerate(self.blocks):   #vit use the their implemented attention,so the batchszie is the first.
...
@register_model
def vit_ce_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vit_ce_base_patch16_224_' + pretrain_type
...
@register_model
def vit_ce_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_ce_large_patch16_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
...
@register_model
def vit_ce_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vit_ce_huge_patch14_224_' + pretrain_type
    if pretrain_type == 'scratch':
        pretrained = False
        return model
    model.default_cfg = default_cfgs[cfg_type]
