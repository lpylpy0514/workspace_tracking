...
class EfficientViT(torch.nn.Module):
    def __init__(self, template_size=128,
        ...
        # Patch embedding
        self.patch_embed = torch.nn.Sequential(Conv2d_BN(in_chans, embed_dim // 8, 2, 2, 0), torch.nn.Hardswish(),
                                               Conv2d_BN(embed_dim // 8, embed_dim // 4, 2, 2, 0), torch.nn.Hardswish(),
                                               Conv2d_BN(embed_dim // 4, embed_dim // 2, 2, 2, 0), torch.nn.Hardswish(),
                                               Conv2d_BN(embed_dim // 2, embed_dim, 2, 2, 0))
        key_dim = embed_dim // (4 * num_heads)
